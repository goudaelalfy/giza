<?php
/**
 * Staticpagebanner controller file.
 *
 * Contains controller class of the static_page_banner table.
 *
...
/**
 * Controller class of the static_page_banner table.
 *
 * This is the controller class of the static_page_banner table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
 */
class Staticpagebanner extends My_Controller
{ 	
	/**
	 * store this controller static_page_banner screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller static_page_banner table name.
	 *
	 * @var string
	 * @access public
	 */
	 ...
	 * @access public
	 */
	public $table_fields="'id',  'page_code', 'banner_image_thumbs', 'banner_files', 'banner_file_selected',  'title',  'title_ar', 'seo_words', 'seo_words_ar', 'brief', 'brief_ar', 'body', 'body_ar'";
	 ...
	 * @access public
	 */
	public $table_fields_to_display=" id,  page_code, banner_image_thumb_selected,  title,  title_ar, approved ";
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/staticpagebanner/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Static_page_banner_model->get_count($this->table);
		...
		$data['rows'] = $this->Static_page_banner_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/static_page_banner/table", $data);
	}
	...
	{	
		...
		$this->Static_page_banner_model->update($this->table, $id, $data);
		$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
		
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/staticpagebanner/index');
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Static_page_banner_model->update($this->table, $del_id, $data);
	...
	/**
	 * Method check redundency of page_code.
	 * 
	 * Method used to check redundency of page_code py passing id as paramter to ignore. 
	 *
	 * @param   int
	 * @access	public
	 */
	public function page_code_redundency($id)
	{
		$page_code = $_POST['page_code'] ;
		$page_code_count=$this->Static_page_banner_model->get_count_by_page_code($this->table,$id,$page_code);
		
			if($page_code_count>0)
			{
				return true;
			}
	...
	/**
	 * Method check availability of page_code.
	 * 
	 * Method used to check availability of page_code called from ajax. 
	 *
	 * @param   int
	 * @access	public
	 */
	public function check_page_code_availability($id)
	{
		if(isset($_POST['page_code']))
		{
			$page_code=$_POST['page_code'];
			$page_code_count=$this->Static_page_banner_model->get_count_by_page_code($this->table,$id,$page_code);
			if($page_code_count>0)
			{
					echo str_replace("###",$page_code, lang('page_code_not_available_error'));
			}
			else
			{
				echo 'OK';
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Static_page_banner_model->get_by_id($this->table, $id);
        } 
        $data['mode']= $mode;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->page_code_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Static_page_banner_model->get_id_by_page_code($this->table,$_POST['page_code']);
				            $data['error']= lang('page_code_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/staticpagebanner/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/staticpagebanner/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/staticpagebanner/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($id==0){
					$this->Static_page_banner_model->insert($this->table, $data['current_row']);
					$id=$this->Static_page_banner_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Static_page_banner_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/staticpagebanner/form/$id/view");
			}
		}
        
		$this->load->view('admin/static_page_banner/form',$data);
	}
