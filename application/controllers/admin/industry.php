 ...
 *
 * Contains controller class of the industry table.
 *
...
/**
 * Controller class of the industry table.
 *
 * This is the controller class of the industry table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	...
	/**
	 * store this controller industry_sub table name.
	 *
	 * @var string
	 * @access private
	 */
	private $table_sub='industry_sub';
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/industry/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Industry_model->get_count($this->table);
		...
		$data['rows'] = $this->Industry_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/industry/table", $data);
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Industry_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Industry_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Industry_model->get_by_id($this->table, $id);
			$data['industry_sub_rows'] = $this->Industry_sub_model->get_all_by_industry_id($this->table_sub, $id); 	
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Industry_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/industry/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/industry/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/industry/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/industry/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/industry/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/industry/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/industry/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/industry/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/industry/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/industry/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($_FILES['menu_icon']['name']!="") {
					...
					if(move_uploaded_file($_FILES["menu_icon"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$menu_icon = '/added/uploads/banner/industry/'.$name_file_timestamp.'.'.$extension;
									
						} else {
								
						if($_FILES['menu_icon']['name']!='') {
							
							$menu_icon=$this->Industry_model->get_menu_icon_by_id($this->table, $id);
				...
				if($id==0){
					$this->Industry_model->insert($this->table, $data['current_row']);
					$id=$this->Industry_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Industry_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/industry/form/$id/view");
			}
		}
        
		$this->load->view('admin/industry/form',$data);
	}
	...
	/**
	 * Method displaying records or rows of industry.
	 * 
	 * Method get all sub of one industry
	 *
	 * @access	public
	 * @param   int industry_id
	 */
	public function sub($industry_id=0, $sub_id=0, $mode='')
	{
		...
		$data['row_industry'] = $this->Industry_model->get_by_id($this->table, $industry_id);
		//$data['rows_sub_industry'] = $this->Industry_sub_model->get_all_by_industry_id($this->table_sub, $id); 	
		
		
		
        if($sub_id!=0){	
			$data['current_row'] = $this->Industry_sub_model->get_by_id($this->table_sub, $sub_id);
        } 
        $data['mode']= $mode;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_sub_redundency($industry_id, $sub_id)) {
					...
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Industry_sub_model->get_id_by_alias($this->table_sub,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/industry/sub/$industry_id/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$sort=$this->Industry_sub_model->get_max_sort($this->table_sub)+1;
				...
				if($sub_id==0){
					$this->Industry_sub_model->insert($this->table_sub, $data['current_row']);
					$sub_id=$this->Industry_sub_model->get_max_id($this->table_sub);
					//----------User Histroy Row ------------------//
					//$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,1,$current_date,$sub_id);		
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				} else {
					$this->Industry_sub_model->update($this->table_sub, $sub_id, $data['current_row']);
					//----------User Histroy Row ------------------//
					//$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$sub_id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/industry/form/".$_POST['industry_id']."/view");
			}
		}
        
		$this->load->view("admin/industry/sub", $data);
...
/**
	 ...
	 * @access	public
	 */
	public function alias_sub_redundency($industry_id, $sub_id)
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Industry_sub_model->get_count_by_industry_and_alias($this->table_sub, $sub_id, $industry_id, $alias);
	 ...
	 * @access	public
	 */
	public function check_alias_sub_availability($industry_id, $sub_id)
	{
		if(isset($_POST['alias']))
		{
			$alias=$_POST['alias'];
	 ...
	 * Delete sub method.
	 * 
	 * Method used to delete industry sub. 
	 *
	 * @access	public
	 * @param   int
	 */
	...
	{	
		...
		$this->Industry_sub_model->update($this->table_sub, $sub_id, $data);
		//$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
	...
	/**
	 * Method sort industry sub.
	 * 
	 * Method used to sort industry sub, ajax. 
	 *
	 * @access	public
	 */
	public function sortsub()
	{
		...
		if ($action == "updateRecordsListings"){
			...
			$update_query = "update industry_sub set sort = " . $listingCounter . " WHERE id = " . $recordIDValue;
			$this->Industry_sub_model->execute_query($update_query);
