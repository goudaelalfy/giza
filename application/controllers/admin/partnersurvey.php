<?php
/**
 * Partnersurvey controller file.
 *
 * Contains controller class of the partner_survey_question table.
 *
...
/**
 * Controller class of the partner_survey_question table.
 *
 * This is the controller class of the partner_survey_question table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
 */
class Partnersurvey extends My_Controller
{ 	
	/**
	 * store this controller partnersurvey screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller partner_survey_question table name.
	 *
	 * @var string
	 * @access private
	 */
	private $table='partner_survey_question';
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/partnersurvey/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Partner_survey_model->get_count($this->table);
		...
		$data['rows'] = $this->Partner_survey_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/partner_survey/table", $data);
	}
	...
	{	
		...
		$this->Partner_survey_model->update($this->table, $id, $data);
		$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
		
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/partnersurvey/index');
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Partner_survey_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Partner_survey_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Partner_survey_model->get_by_id($this->table, $id);
			
        //------------------------ Answers ----------------------------------------
			$answer_rows = $this->Partner_survey_model->get_answers_by_id($id);
		...
		if(isset($_POST['smt_save'])) {
			...
			}
			{
				...
				if($id==0)
				{
					$sort=$this->Partner_survey_model->get_max_sort()+1;
				} else {
					$sort=$this->Partner_survey_model->get_sort_by_id($id);
				}
				...
				if($id==0){
					$this->Partner_survey_model->insert($this->table, $data['current_row']);
					$id=$this->Partner_survey_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Partner_survey_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				...
				foreach($_POST AS $field => $value) {
					if($this->start_with($field,'answer_id_')) {
						...
						//if(isset($_POST['answer_id_'.$counter])) {
							...
							$this->Partner_survey_model->insert_answer($answer_data);
							} else {
								...
								$this->Partner_survey_model->update_answer($answer_id,$answer_data);
				...
				if($arr_answers) {
					foreach($arr_answers as $key=>$value) {
						$this->Partner_survey_model->delete_answer_by_question_and_id($id,$value);
					}
				}
				...
				redirect(base_url().$this->lang->lang()."/".ADMIN."/partnersurvey/form/$id/view");
			}
		}
        
		$this->load->view('admin/partner_survey/form',$data);
	}
	
	function sort($type, $id)
	{
		$row=$this->Partner_survey_model->get_by_id($this->table, $id);
		$sort=$row->sort;
		
		if($type=='up') {
			$rowbeforeafter=$this->Partner_survey_model->get_by_sort_less($sort);
			...
			$rowbeforeafter=$this->Partner_survey_model->get_by_sort_more($sort);
			$rowbeforeafter_sort=$rowbeforeafter->sort;
			$rowbeforeafter_id=$rowbeforeafter->id;
			$rowbeforeafter_sort=$sort;
		...
		$result=$this->Partner_survey_model->update($this->table, $id, $data);
		$result=$this->Partner_survey_model->update($this->table, $rowbeforeafter_id, $rowbeforeafter_data);
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/partnersurvey/index');
	}
