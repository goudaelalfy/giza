 ...
 *
 * Contains controller class of the menu table.
 *
...
/**
 * Controller class of the menu table.
 *
 * This is the controller class of the menu table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller menu screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller menu table name.
	 *
	 * @var string
	 * @access public
	 */
	 ...
	 * @access public
	 */
	public $table_fields_to_display=" id,  title,  title_ar ";
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/menu/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Menu_model->get_count($this->table);
		...
		$data['rows'] = $this->Menu_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/menu/table", $data);
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Menu_model->update($this->table, $del_id, $data);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Menu_model->get_by_id($this->table, $id);
			$data['menu_link_rows'] = $this->Menu_link_model->get_all_by_menu_id($id); 	
		...
		if(isset($_POST['smt_save'])) {
			if($id!=0) {
			...
			}
			 {
				...
				if($id==0){
					$this->Menu_model->insert($this->table, $data['current_row']);
					$id=$this->Menu_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Menu_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/menu/form/$id/view");
			}
		}
	...
	/**
	 * Method sort menu link.
	 * 
	 * Method used to sort menu link, ajax. 
	 *
	 * @access	public
	*/
	public function sortlink()
	{
		...
		if ($action == "updateRecordsListings"){
			...
			$update_query = "update menu_link set sort = " . $listingCounter . " WHERE id = " . $recordIDValue;
			$this->Menu_link_model->execute_query($update_query);
