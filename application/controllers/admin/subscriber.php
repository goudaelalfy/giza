 ...
 *
 * Contains controller class of the subscriber table.
 *
...
/**
 * Controller class of the subscriber table.
 *
 * This is the controller class of the subscriber table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller subscriber screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller subscriber table name.
	 *
	 * @var string
	 * @access public
	 */
	 ...
	 * @access public
	 */
	public $table_fields_to_display=" `id`,  `name`,  `email`,  `company`, `position`, approved ";
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/subscriber/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Subscriber_model->get_count($this->table);
		...
		$data['rows'] = $this->Subscriber_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/subscriber/table", $data);
	}
	...
	{	
		...
		$this->Subscriber_model->update($this->table, $id, $data);
		$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
		
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/subscriber/index');
	}
	...
	public function submit_display()
	{
		if(isset($_POST['smt_export'])) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				$subscriber_rows=$this->Subscriber_model->get_by_array($export_ids_arr);
				
				$html= "<table class='table table-striped table-bordered bootstrap-datatable datatable' border='1'>";
				$index=0;
				foreach ($subscriber_rows as $record)
				{
						$current_row_id=0;
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Subscriber_model->update($this->table, $del_id, $data);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Subscriber_model->get_by_id($this->table, $id);
			...
			if($id!=0) {
			...
			}
			 {
				...
				if($_FILES['logo']['name']!="") {
					...
					$uplad_path_file=getcwd().'/added/uploads/logo/subscriber/' . $name_file_timestamp.'.'.$extension;
					...
					if(move_uploaded_file($_FILES["logo"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$logo = '/added/uploads/logo/subscriber/'.$name_file_timestamp.'.'.$extension;
									
						} else {
								
						if($_FILES['logo']['name']!='') {
							
							$logo=$this->Subscriber_model->get_logo_by_id($this->table, $id);
				...
				*/
				...
				if($id==0){
					...
					$this->Subscriber_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				...
				redirect(base_url().$this->lang->lang()."/".ADMIN."/subscriber/form/$id/view");
				}
			}
		}
        
		$this->load->view('admin/subscriber/form',$data);
	}
