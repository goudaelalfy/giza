 ...
 *
 * Contains controller class of the gallery table.
 *
...
/**
 * Controller class of the gallery table.
 *
 * This is the controller class of the gallery table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller gallery screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller gallery table name.
	 *
	 * @var string
	 * @access public
	 */
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/gallery/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Gallery_model->get_count($this->table);
		...
		$data['rows'] = $this->Gallery_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Gallery_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Gallery_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Gallery_model->get_by_id($this->table, $id);
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					...
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Gallery_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/gallery/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/gallery/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/gallery/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/gallery/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/gallery/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/gallery/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/gallery/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/gallery/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/gallery/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/gallery/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($_FILES['icon']['name']!="") {
					...
					if(move_uploaded_file($_FILES["icon"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$icon = '/added/uploads/banner/gallery/'.$name_file_timestamp.'.'.$extension;
									
						} else {
								
						if($_FILES['icon']['name']!='') {
							
							$icon=$this->Gallery_model->get_icon_by_id($this->table, $id);
				...
				if($id==0){
					$this->Gallery_model->insert($this->table, $data['current_row']);
					$id=$this->Gallery_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,1,$current_date,$id);		
					//---------------------------------------------//
					
					/**
					 * Create directory gallery for images.
					 */
					mkdir(getcwd().'/added/uploads/gallery/'.$id, 0777);
				
					$this->session->set_userdata('message_session',lang('saved_successfully'));
					
						
				} else {
					$this->Gallery_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					
					if(!file_exists(getcwd().'/added/uploads/gallery/'.$id)) {
						/**
					 * Create directory gallery for images.
					 */
					mkdir(getcwd().'/added/uploads/gallery/'.$id, 0777);
				...
				redirect(base_url().$this->lang->lang()."/".ADMIN."/gallery/form/$id/view");
			}
		}
