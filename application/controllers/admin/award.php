 ...
 *
 * Contains controller class of the award table.
 *
...
/**
 * Controller class of the award table.
 *
 * This is the controller class of the award table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller award screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller award table name.
	 *
	 * @var string
	 * @access private
	 */
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/award/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Award_model->get_count($this->table);
		...
		$data['rows'] = $this->Award_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/award/table", $data);
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Award_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Award_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Award_model->get_by_id($this->table, $id);
        } 
        $data['mode']= $mode;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Award_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/award/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/award/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["image"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$images = '/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
							$image_thumbs= '/added/uploads/award/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$image_selected = '/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
							$image_thumb_selected= '/added/uploads/award/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$image_thumbs = '/added/uploads/award/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$image_thumbs = $_POST['hdn_image_thumbs'].',,,'.'/added/uploads/award/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_images']=='') {
								$images = '/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
							} else {
								$images = $_POST['hdn_images'].',,,'.'/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($_FILES['award_file']['name']!="") {
					$userfile_name = $_FILES['award_file']['name']; // file name  
					$userfile_tmp  = $_FILES['award_file']['tmp_name']; // actual location  
					$userfile_size  = $_FILES['award_file']['size']; // file size  
					$userfile_type  = $_FILES['award_file']['type']; // mime type of file sent by browser. PHP doesn't check it.  
					$userfile_error  = $_FILES['award_file']['error'];
									
					$extension = end(explode('.', $_FILES['award_file']['name']));
					/**
					...
					//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
					if(move_uploaded_file($_FILES["award_file"]["tmp_name"], $uplad_path_file))
					{
						
						if($id==0) {
							$award_file = '/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
									
						} else {
								
						if($_FILES['award_file']['name']!='') {
							
							$award_file=$this->Award_model->get_award_file_by_id($this->table, $id);
							$award_file_path=getcwd().$award_file;
							if(isset($award_file_path) && $award_file_path!=getcwd()) {
							unlink($award_file_path);
							}
							$award_file = '/added/uploads/award/'.$name_file_timestamp.'.'.$extension;
				...
				if($id==0)
				{
					$sort=$this->Award_model->get_max_sort()+1;
				} else {
					$sort=$this->Award_model->get_sort_by_id($id);
				}
				...
				if($id==0){
					$this->Award_model->insert($this->table, $data['current_row']);
					$id=$this->Award_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Award_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/award/form/$id/view");
			}
		}
        
		$this->load->view('admin/award/form',$data);
	}
	
	function sort($type, $id)
	{
		$row=$this->Award_model->get_by_id($this->table, $id);
		$sort=$row->sort;
		
		if($type=='up') {
			$rowbeforeafter=$this->Award_model->get_by_sort_less($sort);
			...
			$rowbeforeafter=$this->Award_model->get_by_sort_more($sort);
			$rowbeforeafter_sort=$rowbeforeafter->sort;
			$rowbeforeafter_id=$rowbeforeafter->id;
			$rowbeforeafter_sort=$sort;
		...
		$result=$this->Award_model->update($this->table, $id, $data);
		$result=$this->Award_model->update($this->table, $rowbeforeafter_id, $rowbeforeafter_data);
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/award/index');
