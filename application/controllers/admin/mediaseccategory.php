<?php
/**
 * Mediaseccategory controller file.
 *
 * Contains controller class of the media_section_category table.
 *
...
/**
 * Controller class of the media_section_category table.
 *
 * This is the controller class of the media_section_category table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
 */
class Mediaseccategory extends My_Controller
{ 	
	/**
	 * store this controller media_section_category screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller media_section_category table name.
	 *
	 * @var string
	 * @access public
	 */
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/mediaseccategory/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Media_section_category_model->get_count($this->table);
		...
		$data['rows'] = $this->Media_section_category_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/media_section_category/table", $data);
	}
	...
	{	
		...
		$this->Media_section_category_model->update($this->table, $id, $data);
		$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
		
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/mediaseccategory/index');
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Media_section_category_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Media_section_category_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Media_section_category_model->get_by_id($this->table, $id);
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Media_section_category_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/mediaseccategory/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/media_section_category/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/media_section_category/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/media_section_category/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($_FILES['icon']['name']!="") {
					...
					if(move_uploaded_file($_FILES["icon"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$icon = '/added/uploads/banner/media_section_category/'.$name_file_timestamp.'.'.$extension;
									
						} else {
								
						if($_FILES['icon']['name']!='') {
							
							$icon=$this->Media_section_category_model->get_icon_by_id($this->table, $id);
				...
				if($id==0){
					$this->Media_section_category_model->insert($this->table, $data['current_row']);
					$id=$this->Media_section_category_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Media_section_category_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/mediaseccategory/form/$id/view");
			}
		}
        
		$this->load->view('admin/media_section_category/form',$data);
	}
