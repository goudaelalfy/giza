 ...
 *
 * Contains controller class of the subsidiary table.
 *
...
/**
 * Controller class of the subsidiary table.
 *
 * This is the controller class of the subsidiary table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller subsidiary screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller subsidiary table name.
	 *
	 * @var string
	 * @access private
	 */
	private $table='subsidiary';
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/subsidiary/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Subsidiary_model->get_count($this->table);
		...
		$data['rows'] = $this->Subsidiary_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/subsidiary/table", $data);
	}
	...
	{	
		...
		$this->Subsidiary_model->update($this->table, $id, $data);
		$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,3,$current_date,$id);								
		//---------------------------------------------//
		
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/subsidiary/index');
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Subsidiary_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Subsidiary_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Subsidiary_model->get_by_id($this->table, $id);
        } 
        $data['mode']= $mode;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Subsidiary_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/subsidiary/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/subsidiary/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["image"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$images = '/added/uploads/subsidiary/'.$name_file_timestamp.'.'.$extension;
							$image_thumbs= '/added/uploads/subsidiary/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$image_selected = '/added/uploads/subsidiary/'.$name_file_timestamp.'.'.$extension;
							$image_thumb_selected= '/added/uploads/subsidiary/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$image_thumbs = '/added/uploads/subsidiary/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$image_thumbs = $_POST['hdn_image_thumbs'].',,,'.'/added/uploads/subsidiary/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_images']=='') {
								$images = '/added/uploads/subsidiary/'.$name_file_timestamp.'.'.$extension;
							} else {
								$images = $_POST['hdn_images'].',,,'.'/added/uploads/subsidiary/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($id==0){
					$this->Subsidiary_model->insert($this->table, $data['current_row']);
					$id=$this->Subsidiary_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Subsidiary_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				}
				redirect(base_url().$this->lang->lang()."/".ADMIN."/subsidiary/form/$id/view");
			}
		}
        
		$this->load->view('admin/subsidiary/form',$data);
	}
	
	function sort($type, $id)
	{
		$row=$this->Subsidiary_model->get_by_id($this->table, $id);
		$sort=$row->sort;
		
		if($type=='up') {
			$rowbeforeafter=$this->Subsidiary_model->get_by_sort_less($sort);
			...
			$rowbeforeafter=$this->Subsidiary_model->get_by_sort_more($sort);
			$rowbeforeafter_sort=$rowbeforeafter->sort;
			$rowbeforeafter_id=$rowbeforeafter->id;
			$rowbeforeafter_sort=$sort;
		...
		$result=$this->Subsidiary_model->update($this->table, $id, $data);
		$result=$this->Subsidiary_model->update($this->table, $rowbeforeafter_id, $rowbeforeafter_data);
		redirect(base_url().$this->lang->lang().'/'.ADMIN.'/subsidiary/index');
	}
