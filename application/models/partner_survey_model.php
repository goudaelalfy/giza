<?php
/**
 * Partner_survey Model.
 *
 * It is partner_survey model file include the partner_survey database process class Partner_survey_model.
 *
...
/**
 * Partner_survey Model Class.
 *
 * This class manages the processes on the database table partner_survey
 *
 * @package		models
 * @category	Database
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
 */
class Partner_survey_model extends My_Model
{
   	/**
	 * store this partner_survey_question table name.
	 *
	 * @var string
	 * @access public
	 */
	public $table='partner_survey_question';
	 ...
	 * @access public
	 */
	public $partner_survey_answer_table='partner_survey_answer';
	...
	*/
	...
	{
	    $this->db->select('*');
	    $this->db->from('partner_survey_answer');
	...
	function insert_answers($question_id, $data)
	{
	    ...
	    foreach($data as $row) {
	       	$this->db->insert('partner_survey_answer', $row);
	    }
	}
	function delete_answers($question_id)
	{
	    $this->db->where("question_id", $question_id);
    	return $this->db->delete("partner_survey_answer");
	}
	
	function insert_answer($data)
	{
	   	$this->db->insert('partner_survey_answer', $data);
	}
	function update_answer($answer_id, $data)
	{
		$this->db->where('id', $answer_id);
	   	$this->db->update('partner_survey_answer', $data);
	}
