...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/media_section/form.js' > </script>
...
if($mode!='add')
{
	...
	if($mode=='return')
	{
		...
		$media_section_category_id=$current_row['media_section_category_id'];
...
$this_object= new Mediasection();

/**
 * Mediaseccategory object.
 * 
 * Intialize object from Mediaseccategory controller class.
 * @var object.
 */
$this->load->controller(ADMIN.'/Mediaseccategory');
$media_section_category_object= new Mediaseccategory();
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/mediasection/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<h2><i class="icon-edit"></i> <?php echo lang('media_sections')?></h2>
<div class="box-icon">
...
<form class="form-horizontal" id='frm_mediasection_row' name='frm_mediasection_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/mediasection/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/mediasection/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/mediasection/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/mediasection/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('media_section_category'); ?></label>
<div class="controls">
<?php $dropdowns->drpdwn_media_section_category($media_section_category_object, $media_section_category_id,'media_section_category_id',$are_disabled); ?>
<div class="dv_error"  id="dv_media_section_category_id_false" style="display:none;"><?php echo lang('media_section_category_false'); ?></div>
</div>
</div>
