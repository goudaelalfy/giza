...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/industry/form.js' > </script>
...
$(document).ready(function(){ 
	...
	$(function() {
		$("#contentLeft ul").sortable({ opacity: 0.6, cursor: 'move', update: function() {
			...
			$.post("<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/industry/sortsub", order, function(theResponse){
...
if($mode!='add')
{
	...
	else 
	{
		...
		$menu_icon =$current_row->menu_icon;
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/industry/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<h2><i class="icon-edit"></i> <?php echo lang('industries')?></h2>
<div class="box-icon">
...
<form class="form-horizontal" id='frm_industry_row' name='frm_industry_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/industry/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/industry/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/industry/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/industry/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<?php }?>	
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/industry/sub/<?php echo $id;?>/0/add">
    <i class='icon-edit icon-white'></i>  
	<?php  echo $this->lang->line('lnk_add_industry_sub'); ?>
	</a>
	<?php }?>
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('menu_icon'); ?></label>
<div class="controls">
<div style="width:390px" >
<input type="text" id="menu_icon_name" class="input-xlarge focused" readonly="readonly"> 
<div class="file_input_div">
<input type="button" value="<?php echo lang('btn_file_input'); ?>" class="btn btn-info" />
<input type="file" class="file_input_hidden" id="menu_icon" name="menu_icon" onchange="javascript: document.getElementById('menu_icon_name').value = this.value" />								
</div>
<br/>
<?php if($menu_icon!='') {
	$menu_icon_path=base_url().$menu_icon;
	echo "<a href='$menu_icon_path' target='_blank'>".lang('menu_icon')."</a>";
}?>
<br/>
</div>
</div>
...
<?php 
if(isset($industry_sub_rows))
foreach($industry_sub_rows as $industry_sub_row)
{
				
				$sub_id=$industry_sub_row->id;
				$sub_alias=$industry_sub_row->alias;
				$sub_title=$industry_sub_row->title;
				$sub_title_ar=$industry_sub_row->title_ar;
				$sort=$industry_sub_row->sort; 
							
				
				echo"<li id='recordsArray_$sub_id'>
				...
				<a href='".base_url().$this->lang->lang().'/'.ADMIN."/industry/deletesub/$id/$sub_id' class='btn btn-danger' onclick='return delete_confirm(\"".$this->lang->lang()."\")' ><i class='icon-trash icon-white'></i>".lang('btn_delete')."</a>
				<a href='".base_url().$this->lang->lang().'/'.ADMIN."/industry/sub/$id/$sub_id/edit'  class='btn btn-info' ><i class='icon-edit icon-white'></i>".lang('btn_edit')."</a>
				</td></tr></table>
				</li>
				";	
}
