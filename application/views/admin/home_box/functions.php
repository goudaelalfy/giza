...
class Functions {
   ...
   public function __construct() {
	...
	public function display_data_table($array_of_data,$link_to_screen='',$object) 
	{
		...
		<script type='text/javascript'  src='".base_url()."js/admin/home_box/form.js' > </script>
		";
		...
		foreach ($array_of_data as $record)
		{
				...
				foreach ($record as $key=>$value)
				{
					...
					else if($key=='alias' || $key=='title'|| $key=='title_ar' )
					{
					
						$td_width='150px';
						
						echo"<td class='center' width='$td_width'> <a  href='$link_to_screen/form/$current_row_id/view' >$value</a>  </td>";	
					} else if($key=='icon' ) {
					
						$td_width='100px';
						echo"<td class='center' width='$td_width'> <img src='".base_url().$value."' title='$value' width='100px' height='50px' /> </td>";	
					} else if($key=='approved' )
					{
					if($object->session->userdata('user_session')->admin==1) {
						if($value==0) {					
