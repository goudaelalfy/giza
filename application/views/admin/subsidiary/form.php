...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/subsidiary/form.js' > </script>
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/subsidiary/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<h2><i class="icon-edit"></i> <?php echo lang('subsidiaries')?></h2>
<div class="box-icon">
...
<form class="form-horizontal" id='frm_subsidiary_row' name='frm_subsidiary_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_images" id=" hdn_images" value="<?php echo $images; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/subsidiary/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/subsidiary/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/subsidiary/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/subsidiary/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
foreach($image_thumbs_arr as $image_thumb) {
	...
	if($image_thumb!=='') {
	   echo "<div><img src='".base_url().$image_thumb."' title='$image_thumb' width='80px' height='60px'/> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type='radio' name='images' value='$image_thumb' $checked> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; $chk_delete</div>";
	}  	
}
?>
</div>
