...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/page/form.js' > </script>
...
if($mode!='add')
{
	...
	if($mode=='return')
	{
		...
		$page_category_id=	$current_row['page_category_id'];
 ...
 * Pagecat object.
 * 
 * Intialize object from Pagecat controller class.
 * @var object.
 */
$this->load->controller(ADMIN.'/Pagecat');
$pagecat_object= new Pagecat();
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/page/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<form class="form-horizontal" id='frm_page_row' name='frm_page_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/page/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/page/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/page/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/page/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('page_category'); ?></label>
<div class="controls">
<?php $dropdowns->drpdwn_pagecat($pagecat_object, $page_category_id,'page_category_id',$are_disabled); ?>
</div>
</div>
