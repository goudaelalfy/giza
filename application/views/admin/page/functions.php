...
class Functions {
   ...
   public function __construct() {
	...
	public function display_data_table($array_of_data,$link_to_screen='',$object) 
	{
		...
		<script type='text/javascript'  src='".base_url()."js/admin/page/form.js' > </script>
		";
		...
		foreach ($array_of_data as $record)
		{
				...
				foreach ($record as $key=>$value)
				{
					...
					else if($key=='title' || $key=='title_ar')
					{
				...
				echo "
				<a class='btn btn-success' target='_blank' href='".base_url().$object->lang->lang()."/page/content/$current_row_alias'>
										<i class='icon-zoom-in icon-white'></i>  
											".lang('preview')."                                            
