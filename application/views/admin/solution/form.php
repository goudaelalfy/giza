...
$(document).ready(function(){ 
	...
	$(function() {
		$("#contentLeft ul").sortable({ opacity: 0.6, cursor: 'move', update: function() {
			...
			$.post("<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/solution/sortsub", order, function(theResponse){
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/solution/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<form class="form-horizontal" id='frm_solution_row' name='frm_solution_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/solution/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/solution/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/solution/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/solution/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<?php }?>	
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/solution/sub/<?php echo $id;?>/0/add">
    <i class='icon-edit icon-white'></i>  
	<?php  echo $this->lang->line('lnk_add_solution_sub'); ?>
	</a>
	<?php }?>
...
<?php 
if(isset($solution_sub_rows))
foreach($solution_sub_rows as $solution_sub_row)
{
				
				$sub_id=$solution_sub_row->id;
				$sub_alias=$solution_sub_row->alias;
				$sub_title=$solution_sub_row->title;
				$sub_title_ar=$solution_sub_row->title_ar;
				$sort=$solution_sub_row->sort; 
				...
				<a href='".base_url().$this->lang->lang().'/'.ADMIN."/solution/deletesub/$id/$sub_id' class='btn btn-danger' onclick='return delete_confirm(\"".$this->lang->lang()."\")' ><i class='icon-trash icon-white'></i>".lang('btn_delete')."</a>
				<a href='".base_url().$this->lang->lang().'/'.ADMIN."/solution/sub/$id/$sub_id/edit'  class='btn btn-info' ><i class='icon-edit icon-white'></i>".lang('btn_edit')."</a>
				</td></tr></table>
				</li>
				";	
}
