	...
	<div id="container">
		...
		<div class="content">
			...
			<div id="side-bar">
				...
				if($menu_controller_name=='client') {
...
$arr_alpha_clients = array();
$arr_alpha_clients_index = 0;
foreach($arr_alpha as $index=>$alpha)
{
	$style_display='';
	if($arr_alpha_clients_index!=0) {
	...
	$client_rows= $this->Client_model->get_by_name_like($alpha);
	...
	foreach($client_rows as $client_row)
	{
		...
		<tr>
		<td class='bullet'></td>
		<td style='color: #4a4a4a;'>$client_name</td>
		
		<td width='200'>";
		
		$case_study_rows=$this->Case_study_model->get_by_client($client_id);
		if(count($case_study_rows)>0) {
		echo"<a href='".base_url().$this->lang->lang()."/client/casestudy/$client_id' style='text-decoration:none;' class='link'>".lang('case_studies')."</a>";
		}
...
<?php 
$industry_client_rows= $this->Client_model->get_by_industry_id($industry_id);
foreach($industry_client_rows as $industry_client_row) {
$client_id=$industry_client_row->id;

if($this->lang->lang()=='ar') {
	$client_name=$industry_client_row->name_ar;
} else {
	$client_name=$industry_client_row->name;
}
...
?>
                                                                					...
                                                                					<tr>
                                                                                        ...
                                                                                        <td style='color: #4a4a4a;'><?php echo $client_name; ?></td>
<!-- 
 -->
                                                                                        <td width="200">
...
<?php 
$solution_client_rows= $this->Client_model->get_by_solution_id($solution_id);
foreach($solution_client_rows as $solution_client_row) {
$client_id=$solution_client_row->id;

if($this->lang->lang()=='ar') {
	$client_name=$solution_client_row->name_ar;
} else {
	$client_name=$solution_client_row->name;
}
