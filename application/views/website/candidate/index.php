	...
	<div id="container">
		...
		<div class="content">
			...
			<div id="side-bar">
				...
				if($menu_controller_name=='candidate') {
...
$arr_alpha_candidates = array();
$arr_alpha_candidates_index = 0;
foreach($arr_alpha as $index=>$alpha)
{
	$style_display='';
	if($arr_alpha_candidates_index!=0) {
	...
	$candidate_rows= $this->Candidate_model->get_by_name_like($alpha);
	...
	foreach($candidate_rows as $candidate_row)
	{
		...
		<td width='200'>
		<a href='".base_url().$this->lang->lang()."/casestudies/$candidate_id' style='text-decoration:none;' class='link'>".lang('case_studies')."</a>
		 </td>
...
<?php 
$industry_candidate_rows= $this->Candidate_model->get_by_industry_id($industry_id);
foreach($industry_candidate_rows as $industry_candidate_row) {
$candidate_id=$industry_candidate_row->id;

if($this->lang->lang()=='ar') {
	$candidate_name=$industry_candidate_row->name_ar;
} else {
	$candidate_name=$industry_candidate_row->name;
}
...
<!-- 
                                                                                        {if $arr_industries_candidates[index].candidates[index2].have_casestudy eq "yes"}
 -->
                                                                                        <td width="200">

                                                                                        	<a href="<?php echo base_url().$this->lang->lang();?>/casestudies/$candidate_id" style="text-decoration:none;" class="link"><?php lang('case_studies');?></a>
...
<?php 
$solution_candidate_rows= $this->Candidate_model->get_by_solution_id($solution_id);
foreach($solution_candidate_rows as $solution_candidate_row) {
$candidate_id=$solution_candidate_row->id;

if($this->lang->lang()=='ar') {
	$candidate_name=$solution_candidate_row->name_ar;
} else {
	$candidate_name=$solution_candidate_row->name;
}
