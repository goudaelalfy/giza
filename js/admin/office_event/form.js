...
function validate_form(lang) {
	 ...
	 if (document.getElementById('office_ids').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_office_ids_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_office_ids_false');
	 }
	 
	 if (document.getElementById('date_from').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_date_from_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_date_from_false');
	 }
	 
	 if (document.getElementById('date_to').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_date_to_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_date_to_false');
	 }
	 ...
	 if (document.getElementById('name_ar').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_name_ar_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_name_ar_false');
	 }
