...
function validate_form(lang) {
	 ...
	 if (document.getElementById('question').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_question_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_question_false');
	 }
	 
	 if (document.getElementById('question_ar').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_question_ar_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_question_ar_false');
	 }
