...
function validate_form(lang) {
	...
	var error=0;
	 ...
	 if (document.getElementById('paging_no_of_pages').value.replace(/^\s+|\s+$/g, '')=="")
	 {
	 	show('dv_paging_no_of_pages_false');
		 error=error+1;
	 }
	 else if (document.getElementById('paging_no_of_pages').value.replace(/^\s+|\s+$/g, '')!="")
	 {
		 var regexObj = /^\d+$/;
		 if (regexObj.test(document.getElementById('paging_no_of_pages').value)==false) 
		 {
			 show('dv_paging_no_of_pages_false');
			 error=error+1;
		 }
		 else if(document.getElementById('paging_no_of_pages').value.replace(/^\s+|\s+$/g, '')==0)
		 {
			 show('dv_paging_no_of_pages_false');
			 error=error+1;
		 }
	 ...
	 {
		 hide('dv_paging_no_of_pages_false');
	 }
