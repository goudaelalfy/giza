...
function validate_form(lang) {
	 ...
	 {
		 show('dv_description_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_description_false');
	 }
	 
	 if (document.getElementById('description_ar').value.replace(/^\s+|\s+$/g, '')=="")
	 {
		 show('dv_description_ar_false');
		 error=error+1;
	 }
	 else
	 {
		 hide('dv_description_ar_false');
	 }
